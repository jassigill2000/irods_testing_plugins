...
def run_tests(module, result):
    ...
    module.run_command(['sudo', 'su', '-', 'irods', '-c', 'cd tests/pydevtest; python run_tests.py --xml_output --run_specific_test test_irods_auth_plugin_krb'], check_rc=True)

def install_testing_dependencies(module):
    # prepare keytab
    keytab_dir='/var/lib/irods/secrets/kerberos'
    module.run_command(['sudo', 'mkdir', '-p', keytab_dir], check_rc=True)
    module.run_command(['sudo', 'rsync', '-r', '/projects/irods/vsphere-testing/shared_secrets/kerberos/', keytab_dir], check_rc=True)
    module.run_command(['sudo', 'chown', '-R', 'irods', keytab_dir], check_rc=True)
    # dependencies
    module.run_command('sudo apt-get update', check_rc=True)
    packages = ['git', 'libkrb5-dev', 'krb5-user']
